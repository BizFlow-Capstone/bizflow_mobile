...
class S2bExportService {
  ...
  static Future<File?> export({
    ...
    try {
      ...
      final effectiveSections = sectionsData ??
      ...
      final exportRows = _buildExportRows(dataRows, effectiveSections);
  ...
  static List<_ExportRow> _buildExportRows(
    ...
    for (final section in sectionsData.sections) {
      ...
      for (final row in section.rows) {
        ...
        // S2b: skip TNCN tax rows
        if (_isTncnRow(lineType, row.values, row.taxType)) continue;
    ...
    for (final footerRow in sectionsData.footerRows) {
      final lineType = footerRow.lineType.trim().toLowerCase();
      // S2b: skip TNCN footer rows
      if (_isTncnRow(lineType, footerRow.values, footerRow.taxType)) continue;
      exportRows.add(
        _ExportRow(
          lineType: lineType,
  ...
  /// Returns true if a row is TNCN (personal income tax) — excluded from S2b.
  static bool _isTncnRow(
    String lineType,
    Map<String, dynamic> values,
    String? taxType,
  ) {
    if (lineType != 'tax' && lineType != 'tax_line') return false;
    if (taxType != null) {
      final t = taxType.trim().toUpperCase();
      if (t == 'PIT' || t == 'TNCN') return true;
    }
    final label = (values['dien_giai']?.toString() ?? '').toLowerCase();
    return label.contains('tncn') ||
        label.contains('thu nhập cá nhân') ||
        label.contains('thu nhap ca nhan');
  }
  ...
  static void _writeExportRow(
    ...
    switch (row.lineType) {
      ...
      case 'subtotal':
        ...
        if (_isGrandVatTotalLabel(summaryLabel)) {
      ...
      case 'tax_line':
        ...
        if (_isGrandVatTotalLabel(taxLabel)) {
          _applyFooterStyle(sheet, rowIndex: rowIndex);
        } else {
  ...
  static xlsio.Worksheet _ensureSheet(xlsio.Workbook workbook) {
    ...
    return workbook.worksheets.addWithName('s2b');
  }
  ...
  /// Grand total label for S2b is GTGT only (no TNCN).
  static bool _isGrandVatTotalLabel(String label) {
    final normalized = label.trim().toLowerCase();
    return normalized.contains('tổng số thuế') ||
        normalized.contains('tong so thue') ||
        normalized.contains('thuế gtgt') ||
        normalized.contains('thue gtgt');
  }
  ...
  static Future<File?> _save(xlsio.Workbook workbook, String displayName) async {
    ...
    final patchedBytes = _applyS2bHeaderRichText(bytes);
    await file.writeAsBytes(patchedBytes, flush: true);
    return file;
  }

  static List<int> _applyS2bHeaderRichText(List<int> xlsxBytes) {
    try {
      ...
      const firstLine = 'Mẫu số S2b-HKD';
      const secondLine =
          '(Kèm theo Thông tư số 152/2025/TT-BTC ngày 31 tháng 12 năm 2025 của Bộ trưởng Bộ Tài chính)';
