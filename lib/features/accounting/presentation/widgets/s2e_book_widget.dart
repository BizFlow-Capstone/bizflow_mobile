...
class S2eBookWidget extends StatelessWidget {
  ...
  List<BookColumnDto> _effectiveColumns() {
    ...
    return const [
      ...
      BookColumnDto(fieldCode: 'thu_vao', label: 'Thu/Gửi vào', fieldType: 'money'),
      BookColumnDto(fieldCode: 'chi_ra', label: 'Chi/Rút ra', fieldType: 'money'),
    ];
  }
  ...
  DataRow _buildSectionHeaderRow(String label) {
    return DataRow(
      ...
      cells: _buildRowCells({'dien_giai': label}, _boldStyle()),
    );
  }
  ...
  dynamic _resolveCellValue(Map<String, dynamic> values, String fieldCode) {
    ...
    if (code.contains('thu') || code.contains('vao') || code.contains('deposit') || code.contains('income')) {
      return _pickDyn(values, _thuVaoAliases);
    }
    if (code.contains('chi') || code.contains('rut') || code.contains('ra') || code.contains('withdraw') || code.contains('expense')) {
      return _pickDyn(values, _chiRaAliases);
    }
  ...
  String _formatCellValue(dynamic value, String fieldCode) {
    ...
    if (code.contains('thu') || code.contains('chi') || code.contains('amount') || code.contains('vao') || code.contains('ra')) {
